...
我们在上一篇（[MIT 6.828课程解析（四） Lecture 1：sh.c分析](https://costa-na.github.io/2016/08/08/MIT-6828-Solution-Simplified-Shell/)）已经详细分析了sh.c的结构和逻辑，在分析的最后，我们可以看到`runcmd`这个函数缺少对三种不同命令的处理逻辑，因此，我们在这篇文章中将会补全这些缺失的逻辑代码，让sh.c可以完整的编译运行起来。
...
这里需要知道[`close`](http://pubs.opengroup.org/onlinepubs/9699919799/functions/close.html)的功能是关闭并回收一个文件描述符（`fd`），让其能够被之后的`open`或其他文件操作函数使用：
